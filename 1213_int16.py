...
def ZRLE(x): 
     ...
     zrle_flag            = (np.sum(non_zero_size,axis=1) < 574).reshape(-1,1)
...
def BPC(input):
     ...
     result = (result_sum <= 558)  # 494,  558, 622, 686, 750, 814, 878, 942
...
model = fuse_bn_recursively(models.vgg19(weights=models.VGG19_Weights.IMAGENET1K_V1))#.to('cuda')
