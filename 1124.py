...
def Comp(x : torch.Tensor):      # Tensor (128,3,224,224) -> Tensor (64,3,224,224)
    ...
    # bpc_data, bpc_flag = BPC(x_numpy)
    return sr_flag | zrle_flag | bpc_flag
...
def save_outputs_hook(self, layer_id = str) -> Callable:          
    def fn(_, input) :
        with torch.no_grad():
            ...
            x = Comp(Quant_input)
            Comp_output = Decomp(x,Quant_input)
            # print (Quant_input.type)
        ...
        if shape_mul%64 == 0 :
            if ('bn' not in name) :
                if ('downsample' not in name):
                    ...
                    param[:] = DeQuant(Comp_output, scale, zero_p).reshape(Data_shape)    
                    # param[:] = DeQuant(Quant_input, scale, zero_p)    
                else :
                    Quant_input, scale, zero_p = Quant(param,16)
                    param[:] = DeQuant(Quant_input, scale, zero_p)                     
...
loader = DataLoader(dataset= dataset, # dataset
                   batch_size=32,   # batch size power to 2
                   shuffle = False, # false
