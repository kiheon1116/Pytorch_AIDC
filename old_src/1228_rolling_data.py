...
def BPC(input, length):
     ...
     DBP_XOR_SYMBOL = DBP_XOR_packbits.view(np.uint16).reshape(row,-1,1) ######################################################################## length 변경시 uint(length)로 변경해주어야함.
...
def rolling_index(x_torch,flag_torch,length,dtype):
    ...
    # rolling = (~flag_torch)&(lossy_result == 0x0000)
    # rolling_0 = (~flag_torch)&((lossy_result > 0x0000) & (lossy_result < 128)) 
    # rolling_1 = (~flag_torch)&((lossy_result >= 128) & (lossy_result < 256))
    # rolling_2 = (~flag_torch)&((lossy_result >= 256) & (lossy_result < 512))
    # rolling_3 = (~flag_torch)&((lossy_result >= 512) & (lossy_result < 1024))
    # rolling_4 = (~flag_torch)&((lossy_result >= 1024) & (lossy_result < 2048))
    # rolling_5 = (~flag_torch)&((lossy_result >= 2048) & (lossy_result < 4096))
    # rolling_6 = (~flag_torch)&((lossy_result >= 4096) & (lossy_result < 8192))
    # rolling_7 = (~flag_torch)&((lossy_result >= 8192) & (lossy_result < 16384))
    # rolling_8 = (~flag_torch)&((lossy_result >= 16384) & (lossy_result < 32767)) 
    # rolling_9 = (~flag_torch)&(lossy_result == 32767)#.view(np.int16)
    # 음수
...
def shift(x_torch, flag_torch, length, count, dtype):
    # INT7
    # shift_index_per_word = rolling_index(x_torch,flag_torch,length)  # (-1,32) torch.int16
    # others
    ...
    # print("shift_index_per_64",shift_index_per_64[0].dtype,shift_index_per_64[0])
    shift_max = torch.max(shift_index_per_64,dim=2) #(-1,8) int16
    # print("shitf_max",shift_max[0][0])
    shift_min = torch.min(shift_index_per_64,dim=2) #(-1,8) int16
    # print("shitf_min",shift_min[0][0])
    ...
    # print("shift_result",shift.dtype,shift)
    shift_index = shift.expand(-1,count).reshape(shift_shape) # (-1,32) int16    
    return shift_index    

def shift_to_mask(x_torch, dtype):
    shift_block = x_torch
    mask_m1 = (shift_block == -1)|(shift_block == 1) 
    mask_m2 = (shift_block == -2)|(shift_block == 2) 
    mask_m3 = (shift_block == -3)|(shift_block == 3) 
    mask_m4 = (shift_block == -4)|(shift_block == 4) 
    mask_m5 = (shift_block == -5)|(shift_block == 5) 
    mask_m6 = (shift_block == -6)|(shift_block == 6) 
    mask_m7 = (shift_block == -7)|(shift_block == 7) 
    mask_m8 = (shift_block == -8)|(shift_block == 8) 
    mask_m9 = (shift_block == -9)|(shift_block == 9)     
    mask_m10 = (shift_block == -10)|(shift_block == 10) 

    if(dtype==7):
        mask_block = ((mask_m1*0xffff)+(mask_m2*0xfffe)+(mask_m3*0xfffc)+(mask_m4*0xfff8)+(mask_m5*0xfff0)+(mask_m6*0xffe0)+(mask_m7*0xffc0)+(mask_m8*0xff80)+(mask_m9*0xff00)+(mask_m10*0xfe00))
    elif(dtype==8):
        mask_block = ((mask_m1*0xffff)+(mask_m2*0xfffe)+(mask_m3*0xfffc)+(mask_m4*0xfff8)+(mask_m5*0xfff0)+(mask_m6*0xffe0)+(mask_m7*0xffc0)+(mask_m8*0xff80)+(mask_m9*0xff00))
    elif(dtype==9):
        mask_block = ((mask_m1*0xffff)+(mask_m2*0xfffe)+(mask_m3*0xfffc)+(mask_m4*0xfff8)+(mask_m5*0xfff0)+(mask_m6*0xffe0)+(mask_m7*0xffc0)+(mask_m8*0xff80))
    elif(dtype==10):
        mask_block = ((mask_m1*0xffff)+(mask_m2*0xfffe)+(mask_m3*0xfffc)+(mask_m4*0xfff8)+(mask_m5*0xfff0)+(mask_m6*0xffe0)+(mask_m7*0xffc0))
    elif(dtype==11):
        mask_block = ((mask_m1*0xffff)+(mask_m2*0xfffe)+(mask_m3*0xfffc)+(mask_m4*0xfff8)+(mask_m5*0xfff0)+(mask_m6*0xffe0))
    elif(dtype==12):
        mask_block = ((mask_m1*0xffff)+(mask_m2*0xfffe)+(mask_m3*0xfffc)+(mask_m4*0xfff8)+(mask_m5*0xfff0))
    elif(dtype==13):
        mask_block = ((mask_m1*0xffff)+(mask_m2*0xfffe)+(mask_m3*0xfffc)+(mask_m4*0xfff8))
    elif(dtype==14):
        mask_block = ((mask_m1*0xffff)+(mask_m2*0xfffe)+(mask_m3*0xfffc))
    elif(dtype==15):
        mask_block = ((mask_m1*0xffff)+(mask_m2*0xfffe))
...
def Decomp(x_numpy, flag, mask_block, length):
    ...
    #  result = (x_numpy_reshape & 0xff00) + (x_numpy_reshape & 0x00ff)*flag
     ...
     data = torch.tensor(x_numpy_reshape & mask_block_numpy)
    #  print("x_numpy",x_numpy_reshape[0].dtype,x_numpy_reshape[0])#.shape,x_numpy_reshape.dtype)
    #  print("mask_block",mask_block_numpy[0].dtype,mask_block_numpy[0])#.shape,mask_block_numpy.dtype)
    #  print("data",data[0].dtype,data[0])#.shape,data.dtype)
    #  print("(x_numpy_reshape & mask_block_numpy)",(x_numpy_reshape & mask_block_numpy).shape)#.shape,(x_numpy_reshape & mask_block_numpy).dtype)
    #  print("flag",flag.shape,flag.dtype)
     x = (x_numpy_reshape & mask_block_numpy)*(~flag)
    #  print("compute_A",x[0].dtype,x[0])#.shape,x.dtype)
     y =((x_numpy_reshape & 0xffff)*flag)
    #  print("compute_B",y[0].dtype,y[0])    
    #  print("(x_numpy_reshape & mask_block_numpy)",(x_numpy_reshape & mask_block_numpy)*(~flag) + ((x_numpy_reshape & 0xffff)*flag).shape,(x_numpy_reshape & mask_block_numpy)*(~flag) + ((x_numpy_reshape & 0xffff)*flag).dtype)#.shape,(x_numpy_reshape & mask_block_numpy)*(~flag).dtype)
     result = ((x_numpy_reshape & mask_block_numpy)*(~flag)) + ((x_numpy_reshape & 0xffff)*flag)
    #  print("Result",result[0],result.shape, result.dtype)
    #  print("Result_mid",result.astype(np.int16)[0])
    #  print("Total_result",torch.tensor(result.astype(np.int16))[0])#.shape,torch.tensor(result.astype(np.int16)).dtype)
...
for name, param in tqdm(model.named_parameters()):
    ...
    with torch.no_grad():
        ...
        # print("input",Quant_input[0][1:5])
        # Step 2 : Compression 
        flag = Comp(Quant_input,length)        # flag (294*1) bool
        flag_torch = torch.from_numpy(flag).to('cuda')
        # Step 3 : rolling (shift)
        shift_block = shift(Quant_input,flag_torch,length,count,dtype)        # shift (294*32) 4bit
        mask_block = shift_to_mask(shift_block,dtype).cpu()        # mask (294*32) INT16 
        # print("mask_block",mask_block[0][1:5])
        ...
        # param_index = rolling_index(Quant_input, flag_torch, length)
        # # param_save = np.save("/home/kkh/pytorch/param_data/"+ name,param_index.cpu().numpy())
        ...
        # param_comp_index = rolling_index(Comp_output.to('cuda'), flag_torch, length)
        # param_save = np.save("/home/kkh/pytorch/param_lossy_data/"+ name,param_comp_index.cpu().numpy())
...
# param_save = np.save("/home/kkh/pytorch/param_data/total.weight",parameters_index)
# param_save = np.save("/home/kkh/pytorch/param_lossy_data/total.weight",parameters_comp_index)
