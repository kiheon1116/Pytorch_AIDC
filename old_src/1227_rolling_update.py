...
def rolling(x_numpy, flag):
    lossy_result = x_numpy*(~flag)
    print("lossy result",lossy_result.shape, lossy_result.dtype)
    rolling_0 = (((lossy_result >= 0x0000) & (lossy_result < 128)) | ((lossy_result <= -1) & (lossy_result >= -128)))#.view(np.int16)
    rolling_1 = (((lossy_result >= 128) & (lossy_result < 256)) | ((lossy_result <= -129) & (lossy_result >= -256)))#.view(np.int16)
    rolling_2 = (((lossy_result >= 256) & (lossy_result < 512)) | ((lossy_result <= -257) & (lossy_result >= -512)))#.view(np.int16)
    rolling_3 = (((lossy_result >= 512) & (lossy_result < 1024)) | ((lossy_result <= -513) & (lossy_result >= -1024)))#.view(np.int16)
    rolling_4 = (((lossy_result >= 1024) & (lossy_result < 2048)) | ((lossy_result <= -1025) & (lossy_result >= -2048)))#.view(np.int16)
    rolling_5 = (((lossy_result >= 2048) & (lossy_result < 4096)) | ((lossy_result <= -2049) & (lossy_result >= -4096)))#.view(np.int16)
    rolling_6 = (((lossy_result >= 4096) & (lossy_result < 8192)) | ((lossy_result <= -4097) & (lossy_result >= -8192)))#.view(np.int16)
    rolling_7 = (((lossy_result >= 8192) & (lossy_result < 16384)) | ((lossy_result <= -8193) & (lossy_result >= -16384)))#.view(np.int16)
    rolling_8 = (((lossy_result >= 16384) & (lossy_result < 32768)) | ((lossy_result <= -16385) & (lossy_result >= -32768)))#.view(np.int16)
    rolling_9 = ((lossy_result >= 32768) | (lossy_result <= -32769) )#.view(np.int16)
...
def rolling_index(x_torch,flag_torch,length):
...
def shift(x_torch, flag_torch, length, count):
    ...
    shift = shift_max[0].reshape(-1,1) # (-1,1) int16
    shift_index = shift.expand(-1,count).reshape(shift_shape) # (-1,32) int16
    return shift_index    

def shift_to_mask(x_torch):
    shift_block = x_torch
    mask_0 = (shift_block == 0) 
    mask_1 = (shift_block == 1) 
    mask_2 = (shift_block == 2) 
    mask_3 = (shift_block == 3) 
    mask_4 = (shift_block == 4) 
    mask_5 = (shift_block == 5) 
    mask_6 = (shift_block == 6) 
    mask_7 = (shift_block == 7) 
    mask_8 = (shift_block == 8) 
    mask_9 = (shift_block == 9)     
    mask_block = mask_0&0x007f+mask_1&0x00fe+mask_2&0x01fc+mask_3&0x03f8+mask_4&0x07f0+mask_5&0x0fe0+mask_6&0x1fc0+mask_7&0x3f80+mask_8&0x7f00+mask_9&0xfe00
...
for name, param in tqdm(model.named_parameters()):
    ...
    with torch.no_grad():
        ...
        shift_block = shift(Quant_input,flag_torch,length,count)        # shift (294*32) 4bit
        mask_block = shift_to_mask(shift_block).cpu()        # mask (294*32) INT16 
