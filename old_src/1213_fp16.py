...
def ZRLE(x): 
     ...
     zrle_flag            = (np.sum(non_zero_size,axis=1) < 958).reshape(-1,1)
...
def Decomp(flag,x_numpy):
    #  x_numpy_reshape = x_numpy.cpu().numpy().reshape(-1,64).view(np.int16)
     ...
     result = (x_numpy_reshape & 0xfffe) + (x_numpy_reshape & 0x0001)*flag
...
def save_outputs_hook(layer_id) -> Callable:          
    def fn(_, input) :
        with torch.no_grad():
            ...
            input[0][:] = Comp_output.view(dtype=torch.float16).float()
...
if __name__ == '__main__':
    ...
    for name, param in model.named_parameters():
        ...
        with torch.no_grad():
            ...
            # param[:] = DeQuant(Quant_input, scale, zero_p)
            Comp_input = param.half()
            Comp_input_2 = Comp_input.half()
            print(Comp_input_2.dtype)
            # print("before",Comp_input.shape,Comp_input.dtype, Comp_input)
            flag = Comp(Comp_input)
            # print(name, flag)
            # print(name, np.sum(flag) / len(flag.reshape(-1,)))
            
            Comp_output = Decomp(flag, Comp_input_2).reshape(Comp_input_2.shape)
            # print("After",Comp_output.shape, Comp_output.dtype, Comp_output)
            param[:] = Comp_output.view(dtype=torch.float8).float()
