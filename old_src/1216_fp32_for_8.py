...
if __name__ == '__main__':
    ...
    for name, param in model.named_parameters():
        ...
        Comp_input = Data_1d.view(torch.uint8) 
        with torch.no_grad():
            ...
            #     Comp_input[1::4] = 0
            #     Comp_input[2::4] = 0
            #     param[:] = Comp_input.view(dtype=torch.float32)
                 # param[:] = Comp_input
                # continue
            ...
            # Comp_input[2::4] = 0
            # flag = Comp(Comp_input)            
            # Comp_output = Decomp(flag, Comp_input).reshape(Data_shape.shape)
            # print(param.dtype, param.shape)
            # param[:] = Comp_input.view(dtype=torch.float32).float()
