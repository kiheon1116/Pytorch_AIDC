...
def rolling_index(x_torch,flag_torch,length):
    ...
    # print("flag_torch",flag_torch.dtype)
...
def Decomp(x_numpy, flag, mask_block, length):
     ...
     result = (x_numpy_reshape & 0xff10) + (x_numpy_reshape & 0x007f)*flag
...
for name, param in tqdm(model.named_parameters()):
    ...
    with torch.no_grad():
        ...
        # param_index = rolling_index(Quant_input, flag, length)
        # parameters_index = np.append(parameters_index.reshape(-1,).tolist(),param_index.tolist()).astype(np.int16)
        # x = np.save("/home/kkh/pytorch/param_lossy_data"+ name,param_index.cpu().numpy())
...
# x = np.save("/home/kkh/pytorch/param_lossy_data/total.weight",parameters_index)
