...
import torch.fft
from dct import dct, idct, dct1, idct1, dct_2d, idct_2d, dct_3d, idct_3d, LinearDCT, apply_linear_2d, apply_linear_3d
...
def BPC(input, length):
     ...
     DBP_XOR_SYMBOL = DBP_XOR_packbits.view(np.uint32).reshape(row,-1,1) ######################################################################## length 변경시 uint(length)로 변경해주어야함.
...
def rolling_total(x_torch,length,dtype):
    ...
    #     x_torch_value = (x_torch >= 2**(idx+5)) & (x_torch < 2**(idx+6)) | (x_torch < -(2**(idx+5))) & (x_torch >= -(2**(idx+6)))
    ...
    if(dtype==7): #(rolling*11)+
        index = ((rolling_0*1)+(rolling_1*2)+(rolling_2*3)+(rolling_3*4)+(rolling_4*5)+(rolling_5*6)+(rolling_6*7)+(rolling_7*8)+(rolling_8*9)+(rolling_9*10)+(rolling_m0*-1)+(rolling_m1*-2)+(rolling_m2*-3)+(rolling_m3*-4)+(rolling_m4*-5)+(rolling_m5*-6)+(rolling_m6*-7)+(rolling_m7*-8)+(rolling_m8*-9)+(rolling_m9*-10))
    elif(dtype==8):
        index = (((rolling_0|rolling_1)*1)+(rolling_2*2)+(rolling_3*3)+(rolling_4*4)+(rolling_5*5)+(rolling_6*6)+(rolling_7*7)+(rolling_8*8)+(rolling_9*9)+((rolling_m0|rolling_m1)*-1)+(rolling_m2*-2)+(rolling_m3*-3)+(rolling_m4*-4)+(rolling_m5*-5)+(rolling_m6*-6)+(rolling_m7*-7)+(rolling_m8*-8)+(rolling_m9*-9))
    elif(dtype==9):
        index = (((rolling_0|rolling_1|rolling_2)*1)+(rolling_3*2)+(rolling_4*3)+(rolling_5*4)+(rolling_6*5)+(rolling_7*6)+(rolling_8*7)+(rolling_9*8)+((rolling_m0|rolling_m1|rolling_m2)*-1)+(rolling_m3*-2)+(rolling_m4*-3)+(rolling_m5*-4)+(rolling_m6*-5)+(rolling_m7*-6)+(rolling_m8*-7)+(rolling_m9*-8))
    elif(dtype==10):
         index =(((rolling_0|rolling_1|rolling_2|rolling_3)*1)+(rolling_4*2)+(rolling_5*3)+(rolling_6*4)+(rolling_7*5)+(rolling_8*6)+(rolling_9*7)+((rolling_m0|rolling_m1|rolling_m2|rolling_m3)*-1)+(rolling_m4*-2)+(rolling_m5*-3)+(rolling_m6*-4)+(rolling_m7*-5)+(rolling_m8*-6)+(rolling_m9*-7))
    elif(dtype==11):
        index = (((rolling_0|rolling_1|rolling_2|rolling_3|rolling_4)*1)+(rolling_5*2)+(rolling_6*3)+(rolling_7*4)+(rolling_8*5)+(rolling_9*6)+((rolling_m0|rolling_m1|rolling_m2|rolling_m3|rolling_m4)*-1)+(rolling_m5*-2)+(rolling_m6*-3)+(rolling_m7*-4)+(rolling_m8*-5)+(rolling_m9*-6))
    elif(dtype==12):
        index = (((rolling_0|rolling_1|rolling_2|rolling_3|rolling_4|rolling_5)*1)+(rolling_6*2)+(rolling_7*3)+(rolling_8*4)+(rolling_9*5)+((rolling_m0|rolling_m1|rolling_m2|rolling_m3|rolling_m4|rolling_m5)*-1)+(rolling_m6*-2)+(rolling_m7*-3)+(rolling_m8*-4)+(rolling_m9*-5))
    elif(dtype==13):
        index = (((rolling_0|rolling_1|rolling_2|rolling_3|rolling_4|rolling_5|rolling_6)*1)+(rolling_7*2)+(rolling_8*3)+(rolling_9*4)+((rolling_m0|rolling_m1|rolling_m2|rolling_m3|rolling_m4|rolling_m5|rolling_m6)*-1)+(rolling_m7*-2)+(rolling_m8*-3)+(rolling_m9*-4))
    elif(dtype==14):
        index = (((rolling_0|rolling_1|rolling_2|rolling_3|rolling_4|rolling_5|rolling_6|rolling_7)*1)+(rolling_8*2)+(rolling_9*3)+((rolling_m0|rolling_m1|rolling_m2|rolling_m3|rolling_m4|rolling_m5|rolling_m6|rolling_m7)*-1)+(rolling_m8*-2)+(rolling_m9*-3))
    elif(dtype==15):
        index = (((rolling_0|rolling_1|rolling_2|rolling_3|rolling_4|rolling_5|rolling_6|rolling_7|rolling_8)*1)+(rolling_9*2)+((rolling_m0|rolling_m1|rolling_m2|rolling_m3|rolling_m4|rolling_m5|rolling_m6|rolling_m7|rolling_m8)*-1)+(rolling_m9*-2))
    return index.type(torch.int16)


def shift(x_torch,  length, count, dtype):
    x_torch = x_torch.view(torch.int16)
    # lossless + lossy
    ...
    # print("before",shift_index_per_word.shape,shift_index_per_word.dtype)
    ...
    # print("after",shift_index_per_64.shape, shift_index_per_64.dtype)
    if(count !=1):
        ...
        shift_index = shift_index_per_word
    return shift_index    

def shift_to_mask(x_torch, dtype):
    x_shape = x_torch.shape
    
    dtype_filter    = torch.arange(10) < (17 - dtype)  # dtype = 7, [T,T,T,T,T,T,T,T,T,T] = index 기준 [0:9]이므로,
    mask            = torch.tensor([-(1 << i) for i in range(10)]) * dtype_filter          # [0xffff, 0xfffe, 0xfffc, 0xfff8, 0xfff0, 0xffe0, 0xffc0, 0xff80, 0xff00, 0xfe00]
    offset          = torch.tensor([(256 >> (9 - i)) for i in range(10)]) * dtype_filter   # [0x0000, 0x0001, 0x0002, 0x0004, 0x0008, 0x0010, 0x0020, 0x0040, 0x0080, 0x0100]
    
    mask_flag = torch.zeros((10, x_shape[0], x_shape[1]), dtype=torch.int16)
    for idx in range(10):
        mask_flag[idx] = (torch.abs(x_torch) == (idx + 1))  # rolling_value 가 1,-1d이면, idx=0에 저장.
...
def Decomp(x_numpy, flag, mask_block, offset_block, length):
     ...
     offset_block_numpy = offset_block.numpy()
     result = ((x_numpy_reshape & mask_block_numpy+offset_block_numpy)*(~flag)) + ((x_numpy_reshape & 0xffff)*flag)#+offset_block_numpy
...
for name, param in tqdm(model.named_parameters()):
    ...
    with torch.no_grad():
        ...
        # Comp_output = Decomp(Quant_input, flag, mask_block, offset_block, length).reshape(Quant_input.shape)
...
param_save = np.save("/home/kkh/pytorch/data/param_INT10/total.weight",parameters_index)
